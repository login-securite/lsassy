...
class test_impacketconnection(unittest.TestCase):
    ...
    def test_login_dns_error(self):
        self.conn = ImpacketConnection("pixis.hackndo", domain, da_login, da_password, None)
    ...
    def test_is_admin_error(self):
        self.conn = ImpacketConnection(ip_address, domain, usr_login, usr_password, None)
        self.conn.set_logger(self.log)
        self.conn.login()
        ret = self.conn.isadmin()
        self.assertIsInstance(ret, RetCode)
...
class test_dumper(unittest.TestCase):
    ...
    def test_dll_dump_invalid_shell(self):
        ...
        self.assertIsInstance(ret, RetCode)
        self.assertEqual(ERROR_UNDEFINED[1], ret.error_msg)
...
class test_dumper_protected(unittest.TestCase):
    def setUp(self):
        self.log = Logger(is_debug=False, is_quiet=True)
        self.conn = ImpacketConnection(ip_address_protected, domain, da_login, da_password, None)
        self.conn.set_logger(self.log)
        self.conn.login()
