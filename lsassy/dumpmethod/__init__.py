        ...
        if self.content is None:
            ...
            with open(self.path, 'rb') as p:
                try:
                    ...
                    self.logger.info("{} uploaded".format(self.name))
                    self.uploaded = True
                    return True
...
class IDumpMethod:
    ...
    def executor_copy(self, executor):
        ...
        try:
            ...
            self.logger.info("{} copied as {}".format(executor, self._executor_name))
    ...
    def dump(self, dump_path=None, dump_name=None, no_powershell=False, copy=False, exec_methods=None, **kwargs):
        ...
        for e, exec_method in valid_exec_methods.items():
            ...
            if self._file_handle is None:
                self.logger.error("Failed to dump lsass using {}".format(e))
                continue
            self.logger.info("Lsass dumped in C:{}{} ({} Bytes)".format(self.dump_path, self.dump_name, self._file_handle.size()))
